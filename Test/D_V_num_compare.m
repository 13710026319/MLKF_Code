...
CI_Weight = [0.92, 0.9, 0.89]; % 频率越高,CI算法的权重逐步提升,10HZ下为默认0.88
SCI_Weight = [0.9, 0.8, 0.7]; % 10HZ下为0.6
...
for f_idx = 1 : length(imu_update_factors)
    ...
    if ~jump_to_plot
        for idx_veh = 1 : N_veh_tests
            ...
            for n = 1 : Vehicle_num
                ...
                % 初始化各算法
                filters_dekf{n} = DEKF(n, init_state, init_cov, Q_15d, Sigma_a, Sigma_w, dt_imu, CI_Weight(f_idx));
                filters_diekf{n} = DIEKF(n, init_state, init_cov, Q_15d, Sigma_a, Sigma_w, dt_imu, CI_Weight(f_idx));
                filters_v3{n} = DMLKF_V3(n, init_state, init_cov, Q_15d, Sigma_a, Sigma_w, dt_imu, CI_Weight(f_idx));
                filters_v2{n} = DMLKF_V2(n, init_state, init_cov, Q_15d, Sigma_a, Sigma_w, dt_imu, CI_Weight(f_idx));
                filters_v1{n} = DMLKF_V1(n, init_state, init_cov, Q_15d, Sigma_a, Sigma_w, dt_imu, SCI_Weight(f_idx));
                filters_dmlkf{n} = DMLKF(n, init_state, init_cov, Q_15d, Sigma_a, Sigma_w, dt_imu, SCI_Weight(f_idx));
    ...
    %% 3. 结果表格输出（按指定顺序）
    fprintf('\n======================= 多车规模分布式算法性能对比表 (基准: DMLKF) =======================\n');
    fprintf('%-8s | %-20s | %-20s | %-20s | %-20s | %-20s | %-12s\n', ...
    ...
    for i = 1 : N_veh_tests
        ...
        v2 = rmse_all_v2(i);
        v1 = rmse_all_v1(i);
        dm = rmse_all_dmlkf(i); % 基准

        % 计算各算法相对于 DMLKF 的性能提升比例 (即 DMLKF 比该算法降低了百分之多少的误差)
        p_d = (d - dm) / d * 100;
        p_di = (di - dm) / di * 100;
        p_v3 = (v3 - dm) / v3 * 100;
        p_v2 = (v2 - dm) / v2 * 100;
        ...
        % 按照新格式输出：前 6 列为字符串，最后一列 DMLKF 打印纯数值
        fprintf('%-8d | %-20s | %-20s | %-20s | %-20s | %-20s | %-12.4f\n', v, ...
            sprintf('%.4f (%+.2f%%)', d, p_d), ...
    ...
    mean_di = mean(rmse_all_diekf);
    mean_v3 = mean(rmse_all_v3);
    mean_v2 = mean(rmse_all_v2);
    mean_v1 = mean(rmse_all_v1);
    mean_dm = mean(rmse_all_dmlkf); % 均值基准

    % 计算均值行对应的 DMLKF 提升比例
    p_mean_d = (mean_d - mean_dm) / mean_d * 100;
    p_mean_di = (mean_di - mean_dm) / mean_di * 100;
    p_mean_v3 = (mean_v3 - mean_dm) / mean_v3 * 100;
    p_mean_v2 = (mean_v2 - mean_dm) / mean_v2 * 100;
    p_mean_v1 = (mean_v1 - mean_dm) / mean_v1 * 100;

    % 2. 严格按原有列格式打印输出 Average 均值行
    fprintf('%-8s | %-20s | %-20s | %-20s | %-20s | %-20s | %-12.4f\n', 'Average', ...
        sprintf('%.4f (%+.2f%%)', mean_d, p_mean_d), ...
