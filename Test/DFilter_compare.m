...
if exist(save_file, 'file') && all_core_active && ~any_v_active
    ...
    load(save_file); % 载入已保存的 'hist' 和 'Vehicle_num' [2]
    skip_sim = true;
end
...
if ~skip_sim
    ...
    for a_idx = 1 : length(Anc_list)
        ...
        if ~isempty(vars)
            ...
            dmlkf_idx = find(strcmp(active_algs, 'DMLKF'), 1);
            has_dmlkf = ~isempty(dmlkf_idx);
            if has_dmlkf
                dmlkf_avg = active_avg_rmses(dmlkf_idx);
            end

            % 2. 组装已启用算法的各车 RMSE 矩阵
            ...
            % 4. 生成并直观显示简表（转换为 categorical 彻底消除大括号与引号，自动缩窄列宽）
