% =========================================================================
% C_D_Filter_compare.m (4算法基站数量变动性能对比脚本)
% 对比算法：EKF_9D VS IEKF_9D VS CMLKF_15D VS DMLKF_15D(分布式)
% 评测维度：平均欧氏定位误差 (m) 以及相对于 EKF 的提升百分比
% 通信网络：1 - 2 - 3 - 4 - 1 环形双邻居分布式通信网络
% =========================================================================
clc; clear; close all;
...
Anc_list = 5:15;                % 基站数量范围：5到15
N_anc_tests = length(Anc_list);

% 结果保存路径与文件名配置
save_dir = 'E:\SE3_MLKF\Result';
save_name = 'DC_Alogrithms_Anchor_num_compare.mat';
...
if exist(save_path, 'file')
    ...
    rmse_all_dmlkf = zeros(N_anc_tests, 1);
    fprintf('未检测到历史数据，开始执行 4 算法多数据集联合性能评测...\n');
end
...
if ~jump_to_plot
    for idx_anc = 1:N_anc_tests
        ...
        rmse_all_dmlkf(idx_anc) = sum_euc_dmlkf / Vehicle_num;
        
        fprintf('基站数 %d 运行完毕。EKF=%.4fm, IEKF=%.4fm, CMLKF=%.4fm, DMLKF=%.4fm\n', ...
            anc_num, rmse_all_ekf(idx_anc), rmse_all_iekf(idx_anc), rmse_all_cmlkf(idx_anc), rmse_all_dmlkf(idx_anc));
    end
end

%% 3. 数据处理与性能可视化表格输出
fprintf('\n====================================================== 4算法基站数量变动综合性能对比表 ======================================================\n');
fprintf('%-8s | %-15s | %-28s | %-28s | %-28s\n', '基站数', 'EKF误差(m)', 'IEKF误差及提升(降幅)', 'CMLKF误差及提升(降幅)', 'DMLKF(分布式)误差及提升');
fprintf('--------------------------------------------------------------------------------------------------------------------------------------------\n');
for idx_anc = 1:N_anc_tests
    anc_num = Anc_list(idx_anc);
    err_ekf = rmse_all_ekf(idx_anc); err_iekf = rmse_all_iekf(idx_anc);
    err_cmlkf = rmse_all_cmlkf(idx_anc); err_dmlkf = rmse_all_dmlkf(idx_anc);
...
figure('Name', '4-Algorithms Performance Analysis VS Anchor Number', 'Position', [80, 100, 1300, 550]);
...
subplot(1, 2, 1);
grid on; hold on;
plot(Anc_list, rmse_all_ekf,   'b-o',  'LineWidth', 2.0, 'MarkerSize', 7, 'MarkerFaceColor', 'b', 'DisplayName', 'EKF (9D, Centralized)');
plot(Anc_list, rmse_all_iekf,  'g-^',  'LineWidth', 2.0, 'MarkerSize', 7, 'MarkerFaceColor', 'g', 'DisplayName', 'IEKF (9D, Iterative)');
plot(Anc_list, rmse_all_cmlkf, 'r--s', 'LineWidth', 2.2, 'MarkerSize', 7, 'MarkerFaceColor', 'r', 'DisplayName', 'CMLKF (15D)');
plot(Anc_list, rmse_all_dmlkf, 'k-.d', 'LineWidth', 2.2, 'MarkerSize', 7, 'MarkerFaceColor', 'k', 'DisplayName', 'DMLKF (15D, Distributed)');
xlabel('Anchor Number', 'FontSize', 11, 'FontWeight', 'bold');
...
xticks(Anc_list); xlim([Anc_list(1) - 0.5, Anc_list(end) + 0.5]);
...
subplot(1, 2, 2);
grid on; hold on;
plot(Anc_list, pct_imp_all_iekf,  'g-^',  'LineWidth', 2.0, 'MarkerSize', 7, 'MarkerFaceColor', 'g', 'DisplayName', 'IEKF Improvement');
plot(Anc_list, pct_imp_all_cmlkf, 'r--s', 'LineWidth', 2.2, 'MarkerSize', 7, 'MarkerFaceColor', 'r', 'DisplayName', 'CMLKF Improvement');
plot(Anc_list, pct_imp_all_dmlkf, 'k-.d', 'LineWidth', 2.2, 'MarkerSize', 7, 'MarkerFaceColor', 'k', 'DisplayName', 'DMLKF Improvement'); 
xlabel('Anchor Number', 'FontSize', 11, 'FontWeight', 'bold');
...
if ~jump_to_plot
    ...
    save(save_path, 'Anc_list', 'rmse_all_ekf', 'rmse_all_iekf', 'rmse_all_cmlkf', 'rmse_all_dmlkf');
